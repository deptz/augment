...
def update_story_row_with_jira_link(
    ...
    try:
        ...
        if jira_col_index is None:
            ...
            jira_col_index = _add_jira_ticket_column(story_table, rows)
            if jira_col_index is None:
        ...
        # Format as markdown link for Confluence
        link_text = f"[{jira_key}]({jira_url})"
        
        # Clear existing content and add link
        jira_cell.clear()
        jira_cell.string = link_text
...
def _add_jira_ticket_column(table, rows) -> Optional[int]:
    """
